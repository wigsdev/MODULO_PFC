...
const GeovisorCostos = () => {
    ...
    const [selectedYear, setSelectedYear] = useState<number>(1); // 1 = solo Año 1, 2-5 = incluye mantenimiento
        ...
        <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    ...
                    <div>
                        <label className="text-xs font-semibold text-gray-500 uppercase block mb-1">Calcular hasta el Año</label>
                        <select
                            value={selectedYear}
                            onChange={(e) => setSelectedYear(parseInt(e.target.value))}
                            className="w-full text-sm border border-gray-200 rounded px-3 py-2"
                        >
                            <option value={1}>Solo Año 1 (Instalación)</option>
                            <option value={2}>Hasta Año 2</option>
                            <option value={3}>Hasta Año 3</option>
                            <option value={4}>Hasta Año 4</option>
                            <option value={5}>Hasta Año 5</option>
                        </select>
                    </div>
                    <div className="flex items-end">
                        {selectedRecord && mantenimientoRegion && (
                            <div className="bg-green-50 border border-green-200 rounded-lg px-4 py-2 w-full">
                                <p className="text-xs text-green-600 font-semibold">INVERSIÓN TOTAL (Años 1-{selectedYear})</p>
                                <p className="text-xl font-bold text-green-700">
                                    S/ {(selectedRecord.inversionTotal + ((selectedYear - 1) * mantenimientoRegion.costoAnual)).toLocaleString()}
                                </p>
                                {selectedYear > 1 && (
                                    <p className="text-xs text-gray-500">
                                        Año 1: S/ {selectedRecord.inversionTotal.toLocaleString()} + Mant: S/ {((selectedYear - 1) * mantenimientoRegion.costoAnual).toLocaleString()}
                                    </p>
                                )}
                            </div>
                        )}
                        {selectedRecord && !mantenimientoRegion && (
                            <div className="bg-green-50 border border-green-200 rounded-lg px-4 py-2 w-full">
                                <p className="text-xs text-green-600 font-semibold">INVERSIÓN AÑO 1</p>
